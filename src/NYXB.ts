...
import type { Events, Options, Validations } from '../typings'
import type NYXB from '../typings'
import CommandHandler from './command-handler/CommandHandler'
import EventHandler from './event-handler/EventHandler'
import Cooldowns from './util/Cooldowns'
import type DefaultCommands from './util/DefaultCommands'
...
class NYXBCommands {
   ...
   private async init(options: Options) {
      ...
      if (botOwners.length === 0) {
         ...
         if (ownerId && !botOwners.includes(ownerId))
      ...
      this._cooldowns = new Cooldowns(this as unknown as NYXB, {
      ...
      if (featuresDir)
         new FeaturesHandler(this as unknown as NYXB, featuresDir, client)
